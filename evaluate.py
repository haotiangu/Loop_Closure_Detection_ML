        ...
        for m in range(len(QUERY_SETS)):
            for n in range(len(QUERY_SETS)):
                ...
                for x in pair_similarity:
                    similarity.append(x)
        INFERENCE_VECTORS = np.asarray(DATABASE_VECTORS[1])
        INFERENCE_VECTORS.tofile('database_vectors.bin')
        print("feature saved:inference_vectors.bin")
        INFERENCES_VECTORS = np.asarray(QUERY_VECTORS[0])
        INFERENCES_VECTORS.tofile('query_vectors.bin')
        print("feature saved:inference_vectors.bin")
        print()
